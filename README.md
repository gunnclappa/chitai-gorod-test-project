...
Проект написан на `Java` с использованием сборщика `Gradle` и тестовой среды `JUnit5`. Для UI-тестов используется `Selenide`. Для API-тестов используется `REST Assured`. Для Mobile-тестов используются `Selenide` и `Appium`. Запуск тестов выполняется локально или в `Selenoid` / `Browserstack`. Также реализована сборка в `Jenkins` с формированием `Allure`-отчета и отправкой уведомления с результатами в `Telegram` после завершения прогона.
